...
let result2 = sum('최','낙원');
console.log(result2); // 최낙원

//? 2) 모듈화 & 추상화
//: 복잡한 작업을 수행하는 코드를 함수로 분리
// >> 각 함수가 수행하는 작업에 집중

// cf) 추상화: 특정 기능을 하는 공통된 기능을 정의
...
// button1.addEventListener('click', message); //'button'요소에 이벤트 발생 시 message기능 적용
// button2.addEventListener('click', message); //'button'요소에 이벤트 발생 시 message기능 적용
// ==============================
...
// & 3) 화살표 함수: 기존의 function 키워드를 화샇표(=>)로 대체(간결한 작성)
// >> '호이스팅' 적용 x
// +) 항상 익명함수로 작성
...
cnw(); // 최낙원's job is developer

console.log(username);
// console.log(job); - Error(지역 변수는 해당 스커프를 벗어나면 사용 불가)

/*
  함수 선언 방식에 따른 Best Practice

  - 일반적 상황: 함수 선언문 (function + 함수명 o)
  - 조건문 함수 사용: 함수 표현식 (function + 함수명 선택)
